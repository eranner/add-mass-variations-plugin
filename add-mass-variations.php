...
function cpvg_admin_page_callback() {
    ...
    <div class="wrap">
        ...
        <form method="post" action="">
            ...
            <?php wp_nonce_field('cpvg_add_variation_action', 'cpvg_add_variation_nonce'); ?>
    ...
    if (isset($_POST['cpvg_create_variation']) && $_POST['cpvg_create_variation'] == '1') {
        if (!isset($_POST['cpvg_add_variation_nonce']) || !wp_verify_nonce($_POST['cpvg_add_variation_nonce'], 'cpvg_add_variation_action')) {
            wp_die('Invalid request. Please try again.');
        }


        $variation_name = isset($_POST['variation-name']) ? sanitize_text_field($_POST['variation-name']) : '';
        $variation_price = isset($_POST['variation-price']) ? sanitize_text_field($_POST['variation-price']) : '';
        $variation_exclusions = isset($_POST['variation-exclusions']) ? sanitize_text_field($_POST['variation-exclusions']) : '';
        $variation_attribute = isset($_POST['variation-attribute']) ? sanitize_text_field($_POST['variation-attribute']) : '';
        // Validate the inputs
        if (empty($variation_name)) {
            ...
            $variation_array = array_map('sanitize_title', explode(',', $variation_exclusions));
